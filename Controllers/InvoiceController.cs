...
namespace WebENG.Controllers
{
    public class InvoiceController : Controller
    {
        ...
        {
            List<SummaryInvoiceModel> invoice = SummaryInvoice.GetsSummaryENGInvoice(year);
            return Json(invoice);
        }
        ...
        public JsonResult GetSummaryQuarter(int year)
        {
            ...
            double backlog_volume = datas.Where(w => w.type == "backlog" && w.quarter == 0).Select(s => s.job_eng_in_hand).Sum();
            int backlog = datas.Where(w => w.type == "backlog").Select(s => s.job_id).Count();
            double backlog_invoice_volume = datas.Where(w => w.type == "backlog" && w.finished_date.Year == year && w.status == "Finished").Select(s => s.invoice_eng).Sum();
            ...
            int q1 = datas.Where(w => w.type == "now" && w.quarter == 1).Select(s => s.job_id).Count();
            double q1_invoice_volume = datas.Where(w => w.type == "now" && w.quarter == 1 && w.finished_date.Year == year && w.status == "Finished").Select(s => s.invoice_eng).Sum();
            ...
            int q2 = datas.Where(w => w.type == "now" && w.quarter == 2).Select(s => s.job_id).Count();
            double q2_invoice_volume = datas.Where(w => w.type == "now" && w.quarter == 2 && w.finished_date.Year == year && w.status == "Finished").Select(s => s.invoice_eng).Sum();
            ...
            int q3 = datas.Where(w => w.type == "now" && w.quarter == 3).Select(s => s.job_id).Count();
            double q3_invoice_volume = datas.Where(w => w.type == "now" && w.quarter == 3 && w.finished_date.Year == year && w.status == "Finished").Select(s => s.invoice_eng).Sum();
            ...
            int q4 = datas.Where(w => w.type == "now" && w.quarter == 4).Select(s => s.job_id).Count();
            double q4_invoice_volume = datas.Where(w => w.type == "now" && w.quarter == 4 && w.finished_date.Year == year && w.status == "Finished").Select(s => s.invoice_eng).Sum();
