...
namespace WebENG.LeaveServices
{
    public class LevelService : ILevel
    {
        ...
        public int CalcuLevelStep(List<LevelModel> levels, RequestModel request, LeaveTypeModel leave)
        {
            ...
            //bool isLongLeave = false;
            //bool isLongLeave = request.is_two_step_approve ? request.amount_leave_day >= leave.max_consecutive_days : (decimal)((double)request.amount_leave_hour / 8.0) >= leave.max_consecutive_days;
            if (!hasOperation)
            {
                return current + 1;
            }
            if (request.is_two_step_approve)
            {
                ...
                {
                    if (request.amount_leave_day >= leave.max_consecutive_days)
                    {
                        return current + 1;
                    }
                    else
                ...
                {
                    if ((decimal)((double)request.amount_leave_hour / 8.0) >= leave.max_consecutive_days)
                    {
                        return current + 1;
                    }
                    else
        ...
        public List<LevelModel> GetLevelByDepartment(string department)
        {
            ...
            try
            {
                ...
                string strCmd = string.Format($@"WITH l0 AS (SELECT emp_id,
                                                ...
                                                l1 AS (
													...
													WHERE [ELEAVE].dbo.departments.department_name = @department   
                                                ),
                                                l2 AS (
                                                    SELECT [ELEAVE].dbo.[Approvers].emp_id,
                                                           ...
                                                           'Director' as position,
															...
															WHERE [ELEAVE].dbo.[Approvers].department = @department 
                                                ),
                                                l3 AS (
                ...
                if (dr.HasRows)
                {
                    while (dr.Read())
                    {
                        LevelModel level = new LevelModel()
                        {
                            ...
                            department = dr["department"].ToString(),
                            emp_id = dr["emp_id"].ToString(),
                            emp_name_en = dr["name_en"].ToString(),
                            emp_name_th = dr["name_th"].ToString(),
                            email = dr["email"].ToString(),
                            level = dr["level"] != DBNull.Value ? Convert.ToInt32(dr["level"].ToString()) : 0
                        };
                        levels.Add(level);
