        ...
        public Stream ExportData(FileInfo path, List<JobModel> jobs)
        {
            ...
            if (path.Exists)
            {
                using (ExcelPackage p = new ExcelPackage(path))
                {
                    ...
                    {
                        double eng_cost = jobs[i].job_summary.Count > 0 ? jobs[i].job_summary[0].eng_cost : 0;
                        double cis_cost = jobs[i].job_summary.Count > 0 ? jobs[i].job_summary[0].cis_cost : 0;
                        double ais_cost = jobs[i].job_summary.Count > 0 ? jobs[i].job_summary[0].ais_cost : 0;
                        double total_cost = jobs[i].job_summary.Count > 0 ? jobs[i].job_summary.Sum(s => s.totalCost) : 0;
                        double total_invoice = jobs[i].invoices.Sum(s => s.invoice);
                        ...
                        worksheet.Cells["E" + (i + startRows)].Value = jobs[i].responsible;
                        worksheet.Cells["F" + (i + startRows)].Value = eng_cost == 0 ? "NA" : ((int)(Math.Ceiling((total_cost / eng_cost) * 100))).ToString();
                        worksheet.Cells["G" + (i + startRows)].Value = cis_cost == 0 ? "NA" : ((int)(Math.Ceiling((total_cost / cis_cost) * 100))).ToString();
                        worksheet.Cells["H" + (i + startRows)].Value = ais_cost == 0 ? "NA" : ((int)(Math.Ceiling((total_cost / ais_cost) * 100))).ToString();
                        worksheet.Cells["J" + (i + startRows)].Value = jobs[i].status;
                        worksheet.Cells["L" + (i + startRows)].Value = jobs[i].term_payment.down_payment * 0.01;
                        worksheet.Cells["M" + (i + startRows)].Value = jobs[i].term_payment.document_submit * 0.01;
                        worksheet.Cells["N" + (i + startRows)].Value = jobs[i].term_payment.instrument_vendor * 0.01;
                        worksheet.Cells["O" + (i + startRows)].Value = jobs[i].term_payment.instrument_delivered_ctl * 0.01;
                        worksheet.Cells["P" + (i + startRows)].Value = jobs[i].term_payment.system_delivered_ctl * 0.01;
                        worksheet.Cells["Q" + (i + startRows)].Value = jobs[i].term_payment.fat * 0.01;
                        worksheet.Cells["R" + (i + startRows)].Value = jobs[i].term_payment.delivery_instrument * 0.01;
                        worksheet.Cells["S" + (i + startRows)].Value = jobs[i].term_payment.delivery_system * 0.01;
                        worksheet.Cells["T" + (i + startRows)].Value = jobs[i].term_payment.progress_work * 0.01;
                        worksheet.Cells["U" + (i + startRows)].Value = jobs[i].term_payment.installation_work_complete * 0.01;
                        worksheet.Cells["V" + (i + startRows)].Value = jobs[i].term_payment.commissioning * 0.01;
                        worksheet.Cells["W" + (i + startRows)].Value = jobs[i].term_payment.startup * 0.01;
                        worksheet.Cells["X" + (i + startRows)].Value = jobs[i].term_payment.as_built * 0.01;
                        worksheet.Cells["Y" + (i + startRows)].Value = jobs[i].term_payment.warranty * 0.01;
                        worksheet.Cells["Z" + (i + startRows)].Value = jobs[i].term_payment.finished * 0.01;
                        worksheet.Cells["AA" + (i + startRows)].Value = jobs[i].job_eng_in_hand;
                        worksheet.Cells["AB" + (i + startRows)].Value = jobs[i].job_cis_in_hand;
                        worksheet.Cells["AC" + (i + startRows)].Value = jobs[i].job_ais_in_hand;
                        worksheet.Cells["AD" + (i + startRows)].Value = jobs[i].job_in_hand;
            ...
            if (path.Exists)
            {
                using (ExcelPackage p = new ExcelPackage(path))
                {
                    ...
                    for (int i = 0; i < jobs.Count; i++)
                    {
                        ...
                        worksheet.Cells["L" + (i + startRows)].Value = jobs[i].cis_cost;
                        worksheet.Cells["M" + (i + startRows)].Value = jobs[i].ais_cost;
                        worksheet.Cells["N" + (i + startRows)].Value = jobs[i].total_cost;
                        worksheet.Cells["O" + (i + startRows)].Value = jobs[i].remaining_cost;
                        ...
                        worksheet.Cells["P" + (i + startRows)].Value = use_cost;
                        worksheet.Cells["Q" + (i + startRows)].Value = gp;
                        worksheet.Cells["R" + (i + startRows)].Value = jobs[i].md_rate;
                        worksheet.Cells["S" + (i + startRows)].Value = jobs[i].pd_rate;
                        worksheet.Cells["T" + (i + startRows)].Value = jobs[i].factor;
                        worksheet.Cells["U" + (i + startRows)].Value = jobs[i].quotation_no;
                        worksheet.Cells["V" + (i + startRows)].Value = jobs[i].job_type;
                        worksheet.Cells["W" + (i + startRows)].Value = jobs[i].job_in_hand;
                        worksheet.Cells["X" + (i + startRows)].Value = jobs[i].job_eng_in_hand;
                        worksheet.Cells["Y" + (i + startRows)].Value = jobs[i].job_cis_in_hand;
                        worksheet.Cells["Z" + (i + startRows)].Value = jobs[i].job_ais_in_hand;
                        worksheet.Cells["AA" + (i + startRows)].Value = jobs[i].invoices.Sum(s=>s.invoice);
                        worksheet.Cells["AB" + (i + startRows)].Value = jobs[i].eng_invoice;
                        worksheet.Cells["AC" + (i + startRows)].Value = jobs[i].cis_invoice;
                        worksheet.Cells["AD" + (i + startRows)].Value = jobs[i].ais_invoice;
                        worksheet.Cells["AE" + (i + startRows)].Value = jobs[i].due_date.ToString("dd/MM/yyyy");
                        worksheet.Cells["AF" + (i + startRows)].Value = jobs[i].finished_date.ToString("dd/MM/yyyy");
                        worksheet.Cells["AG" + (i + startRows)].Value = jobs[i].warranty_period;
                        worksheet.Cells["AH" + (i + startRows)].Value = jobs[i].bank_guarantee;
                        worksheet.Cells["AI" + (i + startRows)].Value = jobs[i].bg_start.ToString("dd/MM/yyyy");
                        worksheet.Cells["AJ" + (i + startRows)].Value = jobs[i].bg_finish.ToString("dd/MM/yyyy");
